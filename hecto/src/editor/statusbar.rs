...
impl StatusBar{
    ...
    pub fn render(&mut self){
        ...
        if let Ok(size) = Terminal::size() 
        {
            ...
            let position_indicator = self.current_status.mposition_indicator_to_string();
            let remainder_len = self.saturating_sub(beginning.len());
            let status = format!("{beginning}
                {position_indicator}:>remainder_len$");
            let to_print = if status.len() <= self.width{
                status
            }else{
                String::new()
            };
            let result = Terminal::print_inverted_row(self.position.y,&to_print);
            debug_assert!(result.is_ok(),"Failed to Render Status from file statusbar");
